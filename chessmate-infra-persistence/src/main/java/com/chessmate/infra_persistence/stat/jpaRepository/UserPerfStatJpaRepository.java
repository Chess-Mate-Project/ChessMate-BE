...
public interface UserPerfStatJpaRepository extends JpaRepository<UserPerfStatJpaEntity, Long> {
     ...
     */
    @Query("SELECT s FROM UserPerfStatJpaEntity s WHERE s.userId = :userId AND s.platform = :platform ORDER BY s.rating DESC LIMIT 1")
    Optional<UserPerfStatJpaEntity> findTopRatingByUserIdAndPlatform(
